...
MAX_RESULTS_PER_MESSAGE = int(os.environ.get("MAX_RESULTS_PER_MESSAGE", "5"))


async def obtain_results_from_query(query: str) -> dict[str, list[str]]:
    """Obtain the results from the query."""
    ...
    results = [result.text for result in results if "your search. Here" not in result.text]
...
async def query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Answer the user with the real book where the query song can be found."""
    ...
    for book, songs in result.items():
        ...
        message += "\n"
        page_counter += 1
        if page_counter == MAX_RESULTS_PER_MESSAGE:
            await context.bot.send_message(
